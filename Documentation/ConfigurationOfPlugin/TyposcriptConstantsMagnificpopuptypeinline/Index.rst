...
TypoScript Constants – magnificpopup.type.inline
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

This constants-array is used for Display type “Content-Reference” and
“Inline-Content” with Method “inline”. Some important constants may be
...
.. container:: table-row
   ...
   Description
         Makes the inline Magnific Popup available on every page.
